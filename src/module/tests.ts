...
export async function runTests() {
    ...
    let result = await runTestCase(1, level1Message, "default");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(2, level1Message, "HP Threshold 50");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(3, level1Message, "Boss trait");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(4, level1Message, "Level difference 5");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(5, level11rank1Message, "Caster level 11");
    if (!result) {
        return;
    }
    ...
    await setConfigToDefaults();
    await game.settings.set(MODULENAME, Settings.Keys.SpellEffectLevel, "CasterLevel");
    result = await runTestCase(6, level1Message, "Caster level 1");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(7, level11rank1Message, "Caster level 11 better of level/2xrank");
    if (!result) {
        return;
    }
    ...
    await setConfigToDefaults();
    await game.settings.set(MODULENAME, Settings.Keys.SpellEffectLevel, "BetterOfSlotLevelOrCasterLevel");
    result = await runTestCase(8, level1Message, "Caster level 1 better of level/2xrank");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(9, level1Message, "Improve crit fails only");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(10, level1Message, "Improve fails");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(11, level1Message, "Add a bonus(1)");
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(12, level1Message, "Scale with HP");
    if (!result) {
        return;
    }
    ...
    result = await runTestCaseRollTwiceKeepHigher(13, level1Message);
    if (!result) {
        return;
    }
    ...
    result = await runTestCase(14, level11rank3Message, "Half level plus rank");
    if (!result) {
        return;
    }
    ...
    await game.settings.set(MODULENAME, Settings.Keys.SpellEffectLevel, "CasterLevel");
    result = await runTestCase(15, level1Message, "BonusPerLevelDifference");
    if (!result) {
        return;
    }
